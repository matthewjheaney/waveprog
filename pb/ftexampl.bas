REM plots the inverse Fourier Transform of a function
REM from coefficients that you define  ---
REM it is built on a program that plots fnx(fi%,t) versus fny(fi%,t)
REM for fi%=1 to ni%
pi=4*atn(1)
cla=13
REM This tell the screen to do text rather than graphics
screen 0
print "input w (between 0 and .99)"
REM This is the way you get input from the keyboard
input w
REM The lines below just keep you from doing something crazy
if w>.99 then w=.99
if w<0 then w=0
theloop6ft:
cls
...
def fntriangle(t)
 if t<w then fntriangle=t else fntriangle=w*(1-x)/(1-w)
end def

REM DEFINE FOURIER COEFFICIENTS

REM define the appropriate Fourier coefficients
...
REM you can define more functions if you want -- they don't show up
REM unless you reset ni% to 2 or more
if fi%=2 then
fny=fntriangle(t)
end if
end def
...
line (fnxx(0),fnyy(0))-(fnxx(w),fnyy(w)),12
line (fnxx(1),fnyy(0))-(fnxx(w),fnyy(w)),12
...
keyin6ft:
i$=inkey$
if i$="m" or i$="M" then print:print "hit a key":_
   print "to return":print "to menu":goto theloop6ft
goto keyin6ft
