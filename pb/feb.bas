...
k1=2*fnasin((o2-(sin(3*pi/(nyb%+1)/2))^2))
'k2=2*fnasinh(((sin(3*pi/(nyb%+1)/2))^2-o2)^(1/2))
k2=2*fnacosh((o2-(sin(2*pi/(nyb%+1)/2))^2)^(1/2))

a1=1/sin(k1*(nyb%+1))
a2=1/fncosh(k2*(nyb%+1))
'a3=1/fncosh(k3*(nyb%+1))


def fnamem(nx%,ny%,nn%)
if nn%=2 then fnamem=a1*sin(k1*nx%)*sin(3*pi*ny%/(nyb%+1))
'if nn%=2 then fnamem=2*a2*fnsinh(k2*nx%)*sin(2*pi*ny%/(nyb%+1))
if nn%=1 then fnamem=1.5*a2*cos(pi*nx%)*fnsinh(k2*nx%)*sin(2*pi*ny%/(nyb%+1))
end def
...
m=m+1
REM if you want discrete repeats
if m>repeattime then goto thenext1
goto theloopmem2
