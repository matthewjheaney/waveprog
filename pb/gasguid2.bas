$compile chain
' 2D gas in an infinite tube with pressure fixed at ell
scrwidth=400
pi=atn(1)*4
dt=.1
cllp=13
...
def fnx(x,k,kell,i,t)
  if i=1 then fnx=-normx*sin(k*x+om*t)
  if i=0 then fnx=normx*exp(k*x)*cos(om*t)
end def

def fny(x,k,i,t)
  if i=1 then fny=normy*cos(k*x+om*t)
  if i=0 then fny=normy*exp(k*x)*cos(om*t)
end def
...
line (-scrwidth/4,ell+3)-(ell+3,ell+3),cllp
line (-scrwidth/4,-3)-(ell+3,-3),cllp
for x=-scrwidth/4 to ell step dd
