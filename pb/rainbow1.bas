FILENAME$="rainbow1"
'comment out the first end to save files
pi=4*atn(1)
...
def fnx(fi%,t)=t
def fny(fi%,t)
 if fi%=1 then n2=nred:nr=1
 if fi%=2 then n2=nviolet:nr=1
 if fi%=5 then n2=nred:nr=3
 if fi%=4 then n2=nviolet:nr=3
 if fi%=3 then n2=ngreen:nr=1
 if fi%=6 then n2=ngreen:nr=3
 ny1=(2*t+nr*(pi-2*atn(sin(t)/n2/(1-(sin(t)/n2)^2)^(1/2)))-pi)
 fny=ny1
end def
